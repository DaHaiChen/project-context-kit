...
function findProjectRoot(startDir) {
    ...
    while (true) {
        if (existsSync(path.join(current, ".project-context")))
            return current;
...
function okText(text, structuredContent) {
    return {
...
function formatOutput(data, markdown, responseFormat) {
    return okText(responseFormat === "json" ? JSON.stringify(data, null, 2) : markdown, data);
}
function ensureInsideContext(relativePath) {
...
function toContextRelative(absolutePath) {
...
async function listContextFiles(dir = CONTEXT_DIR) {
...
async function loadModules() {
    const data = await readJsonFile("metadata/modules.json", { modules: [] });
    return Array.isArray(data.modules) ? data.modules : [];
}
async function loadContracts() {
    const data = await readJsonFile("metadata/contracts.json", { contracts: [] });
...
async function listProjectFiles(dir = PROJECT_ROOT) {
    const ignored = new Set(["node_modules", "dist", ".git", ".codegraph"]);
    const entries = await fs.readdir(dir, { withFileTypes: true });
    const files = [];
    for (const entry of entries) {
        if (ignored.has(entry.name) || entry.name === ".DS_Store")
            continue;
        const absolute = path.join(dir, entry.name);
        if (entry.isDirectory()) {
...
async function getModuleDoc(module) {
    if (!module.doc)
        return { path: null, content: null, frontmatter: {} };
...
async function buildGraph() {
    ...
    for (const module of graphModules) {
        for (const target of module.depends_on)
            edges.push({ from: module.id, to: target, type: "depends_on" });
        for (const contract of module.contracts)
            edges.push({ from: module.id, to: contract, type: "uses_contract" });
    }
    for (const contract of graphContracts) {
        for (const moduleId of contract.related_modules)
            edges.push({ from: contract.id, to: moduleId, type: "related_module" });
    }
    return {
        version: 1,
...
function graphToTreeMarkdown(graph) {
...
function graphToMermaid(graph) {
    ...
    for (const module of graph.modules)
        lines.push(`  ${safeMermaidId(module.id)}["${module.name}"]`);
    for (const contract of graph.contracts)
        lines.push(`  ${safeMermaidId(contract.id)}["${contract.name}"]`);
    for (const edge of graph.edges)
        lines.push(`  ${safeMermaidId(edge.from)} -->|${edge.type}| ${safeMermaidId(edge.to)}`);
    return `${lines.join("\n")}\n`;
}
...
function summarize(content, query, maxLength) {
...
async function writeGeneratedGraph(graph) {
...
async function checkStaleness() {
    ...
    const sourceFiles = projectFiles.filter((file) => {
        if (file.startsWith(".project-context/"))
            return false;
...
server.registerTool("project_context_search", {
    ...
    try {
        ...
        for (const file of selected) {
            ...
            const haystack = `${file}\n${content}`;
            if (!includesCaseInsensitive(haystack, query))
                continue;
            matches.push({
                path: file,
...
server.registerTool("project_context_get_module", {
    ...
    try {
        ...
        if (!module)
            return errorText(`未找到模块：${module_id}`, "请检查 .project-context/metadata/modules.json 中是否存在该模块。");
        const moduleDoc = await getModuleDoc(module);
        const relatedContracts = include_contracts
            ? await Promise.all((module.contracts ?? []).map(async (id) => {
                const contract = contracts.find((item) => item.id === id);
                if (!contract)
                    return { id, metadata: null, content: null };
...
server.registerTool("project_context_map_files", {
...
server.registerTool("project_context_generate_graph", {
    ...
    try {
        ...
        if (write_files)
            await writeGeneratedGraph(graph);
...
server.registerTool("project_context_check_staleness", {
    ...
    try {
        ...
        const markdown = result.issues.length
            ...
            : "# 文档过期检查报告\n\n未发现明显问题。";
        if (write_report)
            await fs.writeFile(ensureInsideContext("generated/stale-report.md"), `${markdown}\n`);
...
server.registerTool("project_context_sync_docs_from_diff", {
    ...
    try {
        ...
        if (changedFiles.length === 0)
            return errorText("没有可分析的变更文件", "请传入 files，或传入包含 +++ b/path 的 git diff 文本。");
        const [modules, contracts] = await Promise.all([loadModules(), loadContracts()]);
        const suggestions = [];
        ...
        for (const file of changedFiles) {
            for (const module of modules) {
                if ((module.files ?? []).some((pattern) => matchesGlob(file, pattern))) {
                    ...
                    if (module.doc)
                        suggestions.push({ doc: module.doc, reason: `${file} 命中模块 ${module.id} 的代码范围。`, action: "review" });
                    for (const contractId of module.contracts ?? [])
            ...
            for (const contract of contracts) {
                ...
                if (authorityHit || docHit) {
                    ...
                    if (contract.doc)
                        suggestions.push({ doc: contract.doc, reason: `${file} 影响契约 ${contract.id}。`, action: "review" });
                }
            }
        }
        for (const contractId of affectedContracts) {
            ...
            if (contract?.doc)
                suggestions.push({ doc: contract.doc, reason: `受影响模块关联契约 ${contractId}。`, action: "review" });
        }
